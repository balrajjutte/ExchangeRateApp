...
def calculator(base_eur_rate, target_eur_rate, base_code, target_code):
    ...
    moneyout_label["text"] = f"Input:   ({target_code})"
...
root = tk.Tk()
canvas = tk.Canvas(root, height=HEIGHT, width=WIDTH, bg="#F0FFFF", bd=5)
canvas.pack()

base_frame = tk.Frame(root, bg="#FAAFBA", bd=5)
base_frame.place(relx=0.01, rely=0.01, relwidth=0.49, relheight=0.48)
labelbase = tk.Label(base_frame, text="Base Currency", font=("Verdana", 12), bg="#F0FFFF")
...
lbxbase = tk.Listbox(base_frame, font=("Verdana", 12))
...
btnbase = tk.Button(base_frame, text="Select", font=("Verdana", 16), command=datafetch)
btnbase.place(relx=0.4, rely=0.82, relwidth=0.2, relheight=0.18)


target_frame = tk.Frame(root, bg="#FAAFBA", bd=5)
target_frame.place(relx=0.51, rely=0.01, relwidth=0.48, relheight=0.48)

labeltarget = tk.Label(target_frame, text="Target Currency", font=("Verdana", 12), bg="#F0FFFF")
...
lbxtarget = tk.Listbox(target_frame, font=("Verdana", 12))
...
btntarget = tk.Button(target_frame, text="Select", font=("Verdana", 16), command=datafetch)
btntarget.place(relx=0.4, rely=0.82, relwidth=0.2, relheight=0.18)

exchange_frame = tk.Frame(root, bg="#FAAFBA", bd=5)
exchange_frame.place(relx=0.51, rely=0.51, relwidth=0.48, relheight=0.48)
exchange_label = tk.Label(exchange_frame, bg="#F0FFFF", font=("Verdana", 28), text="Exchange Rate:")
exchange_label.place(relheight=1, relwidth=1)

money_frame = tk.Frame(root, bg="#FAAFBA", bd=5)
money_frame.place(relx=0.01, rely=0.51, relwidth=0.49, relheight=0.48)
money_label = tk.Label(money_frame, bg="#F0FFFF", font=("Verdana", 16), text="CASH EXCHANGE")
money_label.place(relheight=0.3, relwidth=0.6)
moneyin_label = tk.Label(money_frame, bg="#F0FFFF", font=("Verdana", 20), text=f"Input:   ()", justify="left", anchor="w")
moneyin_label.place(rely=0.35, relheight=0.30, relwidth=0.6)
moneyout_label = tk.Label(money_frame, bg="#F0FFFF", font=("Verdana", 20), text=f"Output: ()", justify="left", anchor="w")
moneyout_label.place(rely=0.69, relheight=0.30, relwidth=0.6)
entry = tk.Entry(money_frame, bg="#F0FFFF", font=("Verdana", 16))
entry.place(relx= 0.62, rely=0.35, relheight=0.3, relwidth=0.38)
xch = tk.Button(money_frame, bg="#F0FFFF", font=("Verdana", 20), text="XCH", command=lambda : get_exchange(entry.get()))
xch.place(relx= 0.62, relheight=0.3, relwidth=0.38)
moneyoutput_label = tk.Label(money_frame, bg="#F0FFFF", font=("Verdana", 16), justify="left", anchor="w")
